...
Focus Management remains independent from the Task lifecycle. Ending a Focus Session does not automatically complete its associated Task.
...
The system must not allow two active Focus Sessions for the same user.
...
A user must explicitly complete the Task through Task Management.
...
6. Should a Focus Session support a user-provided note?
